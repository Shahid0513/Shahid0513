import java.util.*;
public class EqualSumREvAssesment {
			...
			if (itr.getValue().size() > 1) {
				...
				for (int i = 0; i < itr.getValue().size();
					...
					System.out.print(
							...
							+ ") " +itr.getValue().get(i).first + itr.getValue().get(i).second);
				}
				...
				/*for (int i = 0; i < itr.getValue().size();
						i++) {
					int res =itr.getValue(). get(i).first + itr.getValue().get(i).second;
